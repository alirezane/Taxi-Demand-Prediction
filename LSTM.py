...
if not infer:
    model_number = 1
    model_dir_path = util.make_model_dir("./models/thesis/LSTM", "LSTM")
else:
    model_dir_path = "./models/thesis/LSTM" + "/model-" + str(model_number)
...
if not infer:
    # Build predictor
    predictor = util.Lstm(train_x=dataset.train_X, train_y=dataset.train_y,
...
# Inference Phase
    ...
    dataset.test = util.add_preds(dataset.test, main_y_hat, "LSTM", dataset.regions)

    # De-normalize data
    dataset.test = util.denormalize_data(dataset.test, ['region', 'LSTM'], dataset.mean, dataset.std)
    #
    # Round predictions
    dataset.test = util.round_values(dataset.test, ['LSTM'])

    # Calculate Errors
    util.calculate_errors(dataset.test, target="region", pred="LSTM")

    # Write results out
    util.result_file(dataset.test, model_path=model_dir_path, target="region", model_names=["LSTM"])

    # Write predicted data out
    util.predictions_to_csv(dataset.test, model_dir_path=model_dir_path, file_name="LSTM")
